...
hardcoded 200ms timeout in case no notification service is alive"
  (when (and circ/format-appt-notify-msg
    ...
    (let* ((message (funcall
                     circ/format-appt-notify-msg mins-to-appt msg curr-time))
           (command (funcall
                     circ/format-appt-notify-cmd message)))
...
(message "%s" circ/org-template-alist)
#+END_SRC
** Org sort headlines
...
; Required for TAB to work in terminal mode to cycle org tree visibility
...
(circ/p1 "v d"  'vc-version-ediff                     "version ediff")
...
(circ/p2 "h"    'shrink-window-horizontally           "shrink horz"                         'override)
(circ/p2 "j"    'enlarge-window                       "enlarge vert"                        'override)
(circ/p2 "k"    'shrink-window                        "shrink vert"                         'override)
(circ/p2 "l"    'enlarge-window-horizontally          "enlarge horz"                        'override)
#+END_SRC

** P3
#+BEGIN_SRC emacs-lisp
;;         KEY    FUNCTION                              DESCRIPTION                           MODE
(circ/p3 "f r"  'set-frame-name                       "rename frame")
...
#+BEGIN_SRC emacs-lisp
(defun circ/startup/inbox (switch) (circ/open-inbox))
(add-to-list 'command-switch-alist '("--inbox" . circ/startup/inbox))

(defun circ/startup/agenda (switch) (circ/alias-org-agenda-agenda))
(add-to-list 'command-switch-alist '("--agenda" . circ/startup/agenda))

(defun circ/startup/tasks (switch) (circ/alias-org-agenda-tasks))
(add-to-list 'command-switch-alist '("--tasks" . circ/startup/tasks))

(defun circ/startup/pims (switch) (circ/set-up-agenda-view))
