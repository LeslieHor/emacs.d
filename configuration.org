...
(defun circ/snippet-option-index (option)
  (when (string-match "{{\\([0-9]*\\)\\(:[A-Za-z0-9 \-_]*\\)?}}" option)
    (match-string 1 option)))
(defun circ/snippet-option-name (option)
  (when (string-match "{{\\([0-9]*\\)\\(:[A-Za-z0-9 \-_]*\\)?}}" option)
    (when (match-string 2 option)
      (substring (match-string 2 option) 1))))
  ...
  (let ((snippet-symbol nil)
    ...
    (while placeholder-options
      (let* ((current-placeholder (car placeholder-options))
             (current-index (circ/snippet-option-index current-placeholder))
             (current-name (circ/snippet-option-name current-placeholder))
             (replacement (read-string (format "(%s) %s: "
                                               current-index
                                               (cond (current-name current-name)
                                                     (t ""))))))
        (with-temp-buffer
          (insert snippet)
          (goto-char (point-min))
