  ...
  (let ((string "  ")
    ...
    (dolist (tag (sort tag-alist (lambda (a b) (> (cdr a) (cdr b)))))
      (let ((tag-name (propertize (car tag) 'face 'org-tag))
            (tag-count (cdr tag)))
        (unless (member tag-name '("proj"))
          (let ((tag-string (format "%s (%s)  " tag-name tag-count)))
            (cond ((> (+ line-length (length tag-string)) text-width)
...
(defun circ/org-agenda-filter-tag ()
  (interactive)
  (let* ((potential-tags (set-difference
                          (mapcar 'car (circ/agenda-count-tags))
                          circ/org-agenda-filter-list))
         (new-tag (completing-read "Tag: " potential-tags)))
    (setq circ/org-agenda-filter-list
          (cons new-tag circ/org-agenda-filter-list))
    (org-agenda-filter-apply
     (mapcar (lambda (tag) (concat "+" tag)) circ/org-agenda-filter-list)
     'tag)
...
(defun circ/org-agenda-open-tasks ()
  (interactive)
  (org-agenda nil "T")
  (circ/org-agenda-insert-tag-cloud))
...
(setq dashboard-persistent-links
      '("file:~/.emacs.d/configuration.org"
        ...
        "file:~/pims/cortex/reference/books/read_books_list.org"
...
(evil-define-key 'motion elfeed-search-mode-map
  ...
  "t" 'circ/elfeed-search-filter-tag
  "o" 'nil
...
(circ/p4 "a t"  'circ/org-agenda-open-tasks           "tasks")
...
(evil-define-key 'motion org-agenda-mode-map
  "s" 'circ/org-agenda-filter-tag
  "S" 'circ/org-agenda-show-all-tags)
#+END_SRC
...
(defun circ/startup/tasks (&optional switch) (circ/org-agenda-open-tasks))
