...
(defun leslie/cortex/create-new-file (title)
  (let* ((abs-filepath (concat (file-name-as-directory leslie/cortex/directory)
...
(defun leslie/repeated-regex (regex-list string)
  (let ((result string))
    (dolist (r regex-list)
      (when r
        (let ((regex (car r))
              ...
              (with-temp-buffer
                (insert-file-contents (concat
                                       (file-name-as-directory
                                        leslie/cortex/directory)
                                       file))
                (org-mode)
                (concat (propertize file 'face '(:foreground "grey"))
                        ": "
                        (propertize (org-get-heading t t)
                                    'face '(:foreground "green"))
                        ": "
                        (leslie/repeated-regex
                         '((" *:PROPERTIES: *\n" . "")
                           (" *:Backlinks: .*\n" . "")
                           (" *:END: *\n?" . "")
                           ("\n" . " -- "))
...
created from."
  ...
  (let* ((initial-input (cond ((region-active-p) (buffer-substring start end))
         ...
         (filepath (replace-regexp-in-string ":.*" ""
                    (ivy-read "Cotex link (C-M-j to done): "
                              (leslie/cortex/get-file-previews)
                              :initial-input initial-input)))
         (description (read-string "Description: " initial-input))
         (temp-abs-filepath (concat
                             (file-name-as-directory leslie/cortex/directory)
                             filepath))
    ...
    (unless (file-exists-p abs-filepath)
      (write-region (format "* %s" (capitalize description)) nil
                    abs-filepath))
      ...
      (let ((file-buffer (get-file-buffer abs-filepath)))
        ...
        (org-entry-put nil "Backlinks"
                       (let* ((backlinks (org-entry-get nil "Backlinks"))
                              ...
                              (new-link (format "[[cortex:%s][%s]]"
                                                src-link
                                                current-heading)))
                         (message new-link)
                         (message backlinks)
                         (cond ((string-match-p (regexp-quote new-link)
                                                (cond (backlinks backlinks)
                                                      (t "")))
                                backlinks)
                               (t
                                (cond (backlinks (concat backlinks " " new-link))
                                      (t new-link))))))
        (write-file abs-filepath)))

    (when (region-active-p)
      (delete-region start end))
    (insert (format "[[cortex:%s][%s]]" link description))))
...
(defun leslie/cortex/rename-slide ()
  ...
  (let* ((old-title (org-get-heading t t))
         (old-heading (concat "* " old-title))
         (new-title (read-string "New title: " old-title))
         (file-name (file-name-nondirectory (buffer-file-name)))
         (old-link (concat "[[cortex:" file-name "][" old-title "]"))
         (new-link (concat "[[cortex:" file-name "][" new-title "]"))
         (point-location (point)))
    ;; Go to the beginning of the buffer
    (goto-char (point-min))
    ;; Search for the current heading
    (re-search-forward (regexp-quote old-heading))
    ;; Replace it with the new heading
    ...
    (let (links)
      ...
      (while (re-search-forward "\\[\\[cortex:\\([0-9]*\.org\\)\\]\\[.*?\\]\\]"
                                nil t)
        (push (match-string 1) links))
      ;; For each link, update the backlinks to the new title
      (dolist (f links)
        (message "Updating %s" f)
        (with-temp-buffer
          (let ((org-filepath (concat
                               (file-name-as-directory leslie/cortex/directory)
                               f)))
            (insert-file-contents org-filepath)
            (org-mode)
            (let ((backlinks (org-entry-get nil "Backlinks")))
              (org-entry-put nil "Backlinks"
                             (replace-regexp-in-string
                              (regexp-quote old-link)
                              new-link backlinks)))
            (write-file org-filepath)))))
...
(leslie/p4 "g n"  'leslie/cortex/create-file            "create cortex file")
(leslie/p4 "g r"  'leslie/cortex/rename-slide           "rename slide"                        'org-mode-map)
(leslie/p4 "g f"  'leslie/cortex/search-slides          "search slides")
