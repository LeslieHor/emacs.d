    ...
    (mapcar (lambda (file)
              (with-temp-buffer
                (insert-file-contents (concat
                                       ...
                                       file))
                (org-mode)
                (concat (propertize (replace-regexp-in-string "\.org" "" file)
      ...
      (let ((file-buffer (get-file-buffer abs-filepath)))
        ...
        (org-entry-put nil "Backlinks"
                       ...
                       (let* ((backlinks (org-entry-get nil "Backlinks"))
                              (link (leslie/cortex/format-link id current-heading)))
...
created from."
  ...
  (let* ((initial-input (cond ((region-active-p) (buffer-substring start end))
                             (t "")))
         (id-desc (leslie/cortex/interactive-slide-picker))
         (id (car id-desc))
         (desc (cdr id-desc))
...
(leslie/p4 "g c"  'leslie/cortex/continue               "continue in another slide"           'org-mode-map)
