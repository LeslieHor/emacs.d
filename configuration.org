...
** Cortex
#+BEGIN_SRC emacs-lisp
(defun leslie/cortex/create-file (&optional initial-title)
...
(defun leslie/cortex/search-slides ()
  ...
  (interactive)
  (let* ((search-term (leslie/cortex/interactive-slide-picker))
         (filepath (leslie/cortex/expand-filepath (car search-term))))
    (cond ((not (file-exists-p filepath))
           (message "Slide does not exist. Creating")
           (leslie/cortex/create-file (cdr search-term)))
          (t
           (find-file filepath)))))
...
(defun leslie/bash-history-search()
  (interactive)
  (if (string-equal major-mode "term-mode")
      (progn
        (let* (;; Hacky fix for ivy sorting. Does not work with default
               ...
               (string (completing-read "History: " (leslie/bash-history))))
...
(evil-define-key 'insert term-raw-map (kbd "C-r") 'leslie/bash-history-search)
#+END_SRC
