...
Keyboard close tab function
#+begin_src emacs-lisp
(defun circ/tab-line-close-tab (&optional given-buffer given-window)
  "Close the tab.
Original function `tab-line-close-tab' does not work when invoked
via keyboard, as it relies on a mouse event to find the tab to
close. When invoked via keyboard, the current tab will be the tab
to close."
  (interactive)
  (let* ((window (or given-window (selected-window)))
         (buffer (or given-buffer (current-buffer)))
         (close-function nil))
    (if (eq 1 (length (funcall tab-line-tabs-function)))
        (user-error "Attempted to close sole tab")
...
Override switching functions to switch by visible tab instead of hidden history.
#+begin_src emacs-lisp
(defun circ/tab-line-current-tab-index ()
    (seq-position
     tabs (current-buffer)
     (lambda (tab buffer)
       (if (bufferp tab)
...
(defun circ/tab-line-switch-to-next-tab (&optional mouse-event)
  ...
  (let ((window (and (listp mouse-event) (posn-window (event-start mouse-event)))))
    (with-selected-window (or window (selected-window))
      (let* ((tabs (funcall tab-line-tabs-function))
             (index (1+ (circ/tab-line-current-tab-index)))
             (tab (nth index tabs))
             (buffer (if (bufferp tab) tab (cdr (assq 'buffer tab)))))
        (cond ((bufferp buffer)
               (tab-line-select-tab-buffer buffer))
              ((not buffer)
               (tab-line-select-tab-buffer (car tabs))))))))

(defun circ/tab-line-switch-to-prev-tab (&optional mouse-event)
...
Close other tabs
#+begin_src emacs-lisp
(defun circ/tab-line-close-other-tabs ()
  (interactive)
  (let ((selected-buffer (window-buffer))
        (tabs (funcall tab-line-tabs-function)))
    (dolist (tab tabs)
      (unless (eq selected-buffer tab)
        (circ/tab-line-close-tab tab)))))
#+end_src
    ...
    (cond ((null target-window)
          ...
          (t
           (let ((target-buffer (window-buffer target-window))
             ...
             (if (eq source-buffer target-buffer)
                  (user-error "Both buffers are the same. Skipping transpose.")
                 (set-window-buffer target-window source-buffer)
               (set-window-buffer source-window target-buffer)
               (circ/tab-line-close-tab source-buffer source-window)
               (circ/tab-line-close-tab target-buffer target-window)))
           (if arg
...
If arg is non-nil, the targetted window is selected."
  (let ((target-window (windmove-find-other-window dir))
    ...
    (cond ((null target-window)
          ...
          (t
           (set-window-buffer target-window source-buffer)
           (circ/tab-line-close-tab source-buffer)
           (if arg
               (select-window target-window))))))
...
(defun circ/setup-agenda-view()
  (interactive)
  (circ/open-inbox)
;  (circ/tab-line-close-other-tabs)
...
(setq ledger-accounts-file "~/finances/ledger.ledger")
#+end_src
*** HL Line for Reports
#+begin_src emacs-lisp
...
*** Set up
#+begin_src emacs-lisp
(add-to-list 'load-path "~/.emacs.d/packages/eglot-1.8")
(require 'eglot)
#+end_src
*** Highlight face
The face used to highlight other usages of a symbol.

Draw a box around it to make it clearer than just a bold face.
#+begin_src emacs-lisp
...
(evil-define-key 'normal xref--xref-buffer-mode-map
  "q" 'circ/kill-this-buffer
  "o" 'xref-goto-xref
  "O" 'xref-quit-and-goto-xref
  "v" 'xref-show-location-at-point
  "n" 'xref-next-line
  "p" 'xref-prev-line
  "]" 'xref-next-line
  "[" 'xref-prev-line
...
(defun circ/eldoc-set-evil-quit-keys ()
  (interactive)
  (message "Setting eldoc evil keys")
  (with-current-buffer eldoc--doc-buffer
    (evil-local-set-key 'normal "q" 'quit-window)))

;; No hook exists when launching the eldoc buffer. So, advice is used instead.
(advice-add 'eldoc-doc-buffer :after 'circ/eldoc-set-evil-quit-keys)
#+end_src
** general
#+BEGIN_SRC emacs-lisp
(add-to-list
...
(defun circ/alias-tab-bar-select-tab-1  () (interactive) (tab-bar-select-tab 1))
(defun circ/alias-tab-bar-select-tab-2  () (interactive) (tab-bar-select-tab 2))
(defun circ/alias-tab-bar-select-tab-3  () (interactive) (tab-bar-select-tab 3))
(defun circ/alias-tab-bar-select-tab-4  () (interactive) (tab-bar-select-tab 4))
(defun circ/alias-tab-bar-select-tab-5  () (interactive) (tab-bar-select-tab 5))
(defun circ/alias-tab-bar-select-tab-6  () (interactive) (tab-bar-select-tab 6))
(defun circ/alias-tab-bar-select-tab-7  () (interactive) (tab-bar-select-tab 7))
(defun circ/alias-tab-bar-select-tab-8  () (interactive) (tab-bar-select-tab 8))
(defun circ/alias-tab-bar-select-tab-9  () (interactive) (tab-bar-select-tab 9))
(defun circ/alias-tab-bar-select-tab-10 () (interactive) (tab-bar-select-tab 10))
#+END_SRC
...
(defun circ/p1 (key command description &optional keymap)
  (let* ((prefix-normal   circ/p1-prefix-normal)
         ...
         (prefix-key (cond ((string= key ";") (concat prefix-special ":"))
                           ...
                           ((string= key ".") (concat prefix-special ">"))
                           ((string= key "/") (concat prefix-special "?"))
                           ((string= key "1") (concat prefix-special "!"))
                           ((string= key "2") (concat prefix-special "\""))
                           ((string= key "3") (concat prefix-special "£"))
                           ((string= key "4") (concat prefix-special "$"))
                           ((string= key "5") (concat prefix-special "%"))
                           ((string= key "6") (concat prefix-special "^"))
                           ((string= key "7") (concat prefix-special "&"))
                           ((string= key "8") (concat prefix-special "*"))
                           ((string= key "9") (concat prefix-special "("))
                           ((string= key "0") (concat prefix-special ")"))
...
(circ/p1 "1"    'circ/alias-tab-bar-select-tab-1      "switch to tab 1")
(circ/p1 "2"    'circ/alias-tab-bar-select-tab-2      "switch to tab 2")
(circ/p1 "3"    'circ/alias-tab-bar-select-tab-3      "switch to tab 3")
(circ/p1 "4"    'circ/alias-tab-bar-select-tab-4      "switch to tab 4")
(circ/p1 "5"    'circ/alias-tab-bar-select-tab-5      "switch to tab 5")
(circ/p1 "6"    'circ/alias-tab-bar-select-tab-6      "switch to tab 6")
(circ/p1 "7"    'circ/alias-tab-bar-select-tab-7      "switch to tab 7")
(circ/p1 "8"    'circ/alias-tab-bar-select-tab-8      "switch to tab 8")
(circ/p1 "9"    'circ/alias-tab-bar-select-tab-9      "switch to tab 9")
(circ/p1 "0"    'circ/alias-tab-bar-select-tab-10     "switch to tab 10")
...
(circ/p1 "c"    'delete-window                        "delete window")
...
(circ/p1 "e c"  'completion-at-point                  "completion-at-point"                 'erlang-mode-map)
(circ/p1 "e c"  'completion-at-point                  "completion-at-point"                 'python-mode-map)
...
(circ/p1 "e s}" 'circ/surround-braces                 "braces { }")
(circ/p1 "e"    'nil                                  "edits")
(circ/p1 "f d"    'xref-find-definitions              "goto definition")
(circ/p1 "f r"    'xref-find-references               "find references")
(circ/p1 "f e"    'eldoc-doc-buffer                   "find documentation")
...
;(circ/p1 "w c"  'delete-window                        "close window")
...
(circ/p3 "c"    'tab-bar-new-tab                      "new tab")
(circ/p3 "q"    'tab-bar-close-tab                    "close tab")
...
(circ/p3 "t"    'nil                                  "tabs")
(circ/p3 "t bl" 'nil                                  "tab bar")
(circ/p3 "t br" 'tab-bar-rename-tab                   "rename tab")
(circ/p3 "t l"  'nil                                  "tab line")
;(circ/p3 "t lq" 'circ/tab-line-close-tab              "close tab")
(circ/p3 "q"    'tab-bar-close-tab                    "close tab")
(circ/p3 "c"    'tab-bar-new-tab                      "new tab")
(circ/p3 "p"    'tab-bar-switch-to-prev-tab           "prev tab")
(circ/p3 "n"    'tab-bar-switch-to-next-tab           "next tab")
(circ/p3 "<"    'evil-jump-backward                   "jump back")
(circ/p3 ">"    'evil-jump-forward                    "jump forward")
