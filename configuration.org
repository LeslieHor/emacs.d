  ...
  (when (and circ/format-appt-notify-msg
    ...
    (let* ((head-msg (cond ((listp msg) (car msg))
           ...
           (head-mins (cond ((listp mins-to-appt) (car mins-to-appt))
                            (t mins-to-appt)))
           (message (funcall
                     circ/format-appt-notify-msg head-mins curr-time head-msg))
           (command (funcall
...
** New Scratch Modes
#+BEGIN_SRC emacs-lisp
(defun new-scratch-with-mode (mode &optional idx)
  "Opens a new scratch buffer in given mode. If one already exists,
creates a new one"
  (interactive)
  (let* ((index (cond (idx idx) (t 1)))
         (buffer-name (concat "*" (symbol-name mode) "-scratch-"
                              (int-to-string index) "*")))
    (cond ((not (get-buffer buffer-name))
           (switch-to-buffer buffer-name)
           (funcall mode))
          (t
           (new-scratch-with-mode mode (+ 1 index))))))
...
(require 'elfeed)
(setq elfeed-db-directory "~/pims/rss_elfeed/elfeed/")
#+END_SRC
...
(require 'elfeed-org)
(elfeed-org)
(setq rmh-elfeed-org-files (list "~/pims/rss_elfeed/rss_feeds.org"))
#+END_SRC
...
(defun circ/elfeed-open-in-mpv ()
  (interactive)
  (let ((browse-url-browser-function 'circ/open-in-mpv))
    (elfeed-show-visit)))

(defun circ/elfeed-open-eww-readable()
  (interactive)
  (let ((browse-url-browser-function 'circ/eww-readable))
    (elfeed-show-visit)))

(defun circ/elfeed-open-eww()
  (interactive)
...
(evil-define-key 'motion elfeed-show-mode-map
  ...
  "o" 'nil
  "or" 'circ/elfeed-open-eww-readable
  "oe" 'circ/elfeed-open-eww
  "ob" 'elfeed-show-visit
  "om" 'circ/elfeed-open-in-mpv
  "y" 'elfeed-show-yank
